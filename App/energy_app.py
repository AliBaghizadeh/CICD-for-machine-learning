...
def plot_load_forecast():
    ...
    # Prepare data for multi-series plot (Actual vs Predicted)
    # We melt the dataframe to have a 'Type' column for color mapping
    plot_df = df.melt(
        id_vars=['timestamp'], 
...
with gr.Blocks(title="⚡ Energy Load Forecast - Multi-Model Comparison") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("📊 Monitoring"):
            ...
            with gr.Row():
                # Updated to LinePlot for time series
