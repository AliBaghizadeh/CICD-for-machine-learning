...
def engineer_features(df_imputed):
    ...
    df_feat["weekofyear"] = df_feat.index.isocalendar().week.astype(int) # Ensure int type
    ...
    df_feat["month_sin"] = np.sin(2 * np.pi * df_feat["month"] / 12) # Use 12 for month period
...
if __name__ == "__main__":
    ...
    with open(PROJECT_ROOT/"Results/metrics.txt", "w") as outfile:
        outfile.write(f"Mean Absolute Error = {round(mae, 2)}, R2 Score = {round(r2, 2)}.\n")
    ...
    print(f"Pipeline successfully saved to {MODEL_PATH / 'energy_forecast_pipeline.skops'}")

    # (Optional: Test loading the model)
    # loaded_pipeline = sio.load(MODEL_PATH / "energy_forecast_pipeline.skops", trusted=True)
    # print(f"Model successfully loaded. Type: {type(loaded_pipeline)}")